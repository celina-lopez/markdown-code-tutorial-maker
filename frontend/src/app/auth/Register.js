...
export function Register({onChange}) {
  ...
  const validate = () => {
    ...
    if (!isStrongPassword(user.password, {minLength: 6})) {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(buildRoute('/users/register/'), JSON.stringify(user), {headers: {"Content-Type": "application/json"}}).then((response) => {
      if ('token' in response.data) {
        login(response.data.token);
      }
    }).catch(error => {
      setErrors({errors, invalid: error?.response?.data?.detail || "Email or username is taken."});
    });
  }
  return (
    <div>
